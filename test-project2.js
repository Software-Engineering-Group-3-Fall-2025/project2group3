...
(function () {
  ...
  if (typeof MakeMultiFilter !== 'function') {
    ...
    const filterFunc = MakeMultiFilter(originalArray);

    const secondArray = [1, 2, 3, 4];
    const filterFuncTwo = MakeMultiFilter(secondArray);
  ...
  varDeclared.forEach(function (sym) {
    if (globalThis[sym] !== undefined) {
      console.error('Found my symbol', sym, 'in global namespace');
      p3Message = 'FAILURE';
    }
  });
  ...
  globalThis.addEventListener?.('load', function () {
    ...
    if (p1El) p1El.innerHTML = p1Message;
    if (p2El) p2El.innerHTML = p2Message;
    if (p3El) p3El.innerHTML = p3Message;
  });
}());
